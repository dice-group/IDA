...
export default class IDALineChart extends Component {
	...
	handleListItemClick = (event, index) => {
		...
		if (this.data.series.length === selectedItems.length) {
			this.setState({
				selectAll: true
			});
		}
	...
	handleToggleAll = () => {
		...
		if (!this.state.selectAll) {
			selectedItems = this.data.series.map((i) => i.label);
		}
		this.setState({
					...
					<Grid item xs={12}>
						<div className="m-2">
							<Chip
								...
								className={{
									"mr-2 mt-2": true,
									"not-shown": !this.state.selectAll
								}}
							/>
							{
					...
					<Grid item md={3}>
						<div>
							<List component="nav" dense={true} aria-label="graph legend" className="line-chart-legend">
								<ListItem key="toggle-all" button onClick={(event) => this.handleToggleAll()}>
									<ListItemAvatar>
										<span></span>
									</ListItemAvatar>
