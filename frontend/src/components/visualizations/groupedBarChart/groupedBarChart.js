...
export default class IDAGroupedBarGraph extends Component {
	...
	drawGraph() {
		...
		const y = d3.scaleLinear()
			...
			.rangeRound([(this.height * 0.95) - this.margin.bottom, this.margin.top]);
		...
		const xAxis2 = (g) => (g)
			.attr("transform", `translate(0,${(this.height * 0.95) - this.margin.bottom})`)
			.call(d3.axisBottom(x1).tickSizeOuter(0).tickFormat((v) => v.length > 16 ? v.substring(0, 13) + "..." : v))
			.selectAll("text")
			.attr("x", -15)
			.attr("y", -5)
